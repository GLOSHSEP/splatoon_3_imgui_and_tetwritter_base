...
//#include "windows/aimbot.h"
//#include "windows/triggerbot.h"
...
namespace nya::hid { 
    ...
    HkTrampoline<void, nn::hid::NpadBaseState*, int, const u32&> GetNpadStatesFullKeyHook = hk::hook::trampoline([](nn::hid::NpadBaseState* state, int a, const u32& b) {
        GetNpadStatesFullKeyHook.orig(state, a, b);
        //nya::window::triggerbot::update_npad(state);
        if (toggleInput) disableButtons(state);
        //if (nya::window::aimbot::aimbot_use) disable_L(state);
    });

    HkTrampoline<void, nn::hid::NpadBaseState*, int, const u32&> GetNpadStatesHandHeldHook = hk::hook::trampoline([](nn::hid::NpadBaseState* state, int a, const u32& b) {
        GetNpadStatesHandHeldHook.orig(state, a, b);
        //nya::window::triggerbot::update_npad(state);
        if (toggleInput) disableButtons(state);
        //if (nya::window::aimbot::aimbot_use) disable_L(state);
    });

    HkTrampoline<void, nn::hid::NpadBaseState*, int, const u32&> GetNpadStatesJoyDualHook = ([](nn::hid::NpadBaseState* state, int a, const u32& b) {
        GetNpadStatesJoyDualHook.orig(state, a, b);
        //nya::window::triggerbot::update_npad(state);
        if (toggleInput) disableButtons(state); 
        //if (nya::window::aimbot::aimbot_use) disable_L(state);
    });
    ...
    void install_hooks() {
        GetNpadStatesFullKeyHook.installAtSym<"_ZN2nn3hid13GetNpadStatesEPNS0_16NpadFullKeyStateEiRKj">();
        GetNpadStatesHandHeldHook.installAtSym<"_ZN2nn3hid13GetNpadStatesEPNS0_17NpadHandheldStateEiRKj">();
        GetNpadStatesJoyDualHook.installAtSym<"_ZN2nn3hid13GetNpadStatesEPNS0_16NpadJoyDualStateEiRKj">();
    }
